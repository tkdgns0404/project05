				...
				<c:if test="${not empty resultBoardList}">
					<c:forEach items="${resultBoardList}" var="resultBoard" varStatus="status">
						<tr>
							...
							<td>${resultBoard.title}</td>
							<td>${resultBoard.writer}</td>
							<td>${resultBoard.cnt}</td>
							<td>${resultBoard.likes}</td>
							<td>${resultBoard.hates}</td>
							<td><fmt:formatDate value="${resultBoard.regDate}" pattern="YYYY-MM-dd"/></td>
						</tr>
					</c:forEach>
				</c:if>
			</tbody>
